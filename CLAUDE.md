...
.claude/                              ← framework root (adapter + all logic lives here)
  ...
  core/
    hooks/
      guards/
        pre-guard.sh                  ← PreToolUse guard (risk-scoring engine v2.3.0)
      ...
      runtime/
        ...
        task-tracker.sh               ← TaskCreated/TaskCompleted → .claude/cache/tasks.json (v1.0.2)
        stop-build.sh                 ← skips if project running, retries build 3x, reports failures (v1.4.0)
...
All hooks and commands resolve their runtime path dynamically. No file depends on a hardcoded path. Resolution order:

1. `$CORTEX_ROOT` environment variable (CI, Docker, custom install)
2. `$(pwd)/.claude` — project-local install if present
...
**PreToolUse (`Bash`)** — `guards/pre-guard.sh` (v2.3.0)
...
Runs when a session begins. Detects project type (dotnet > rust > java > node > go > python priority), extracts dependencies, entry points, and folder structure, then writes `.claude/cache/project-profile.json`. Idempotent via fingerprint; skips rewrite if project files are unchanged. Prunes scan cache entries older than 7 days on each run.
...
**TaskCreated / TaskCompleted** — `runtime/task-tracker.sh` (v1.0.2)
Persists task lifecycle events to `.claude/cache/tasks.json`. Fires on both task creation and task completion. Always exits 0.
...
- After editing any hook or scanner, copy `.claude/` to `~/.claude/` or run `/init-cortex`
...
3. Run `/init-cortex` (or copy `.claude/` to `~/.claude/` manually)
